...
////////////////////// VIEWS //////////////////////
gulp.task('viewsClean', function() {
  return del(['build/views/*.html']);
});
...
gulp.task('serve', ['build'], function() {
  ...
  gulp.watch(['app/views/*.html'], ['viewBuild']); // view html changes, reload.
  gulp.watch(['resources/styles/*.css', 'resources/styles/*.scss'], ['cssBuild']); // css or sass changes, concatenate all css/sass, build, reload.
  gulp.watch(['app/*.ts', 'app/model/*.ts'], ['tsBuild']); // typescript files change, compile then reload.
});
...
gulp.task('viewBuild', ['views'], function(){
  browserSync.reload();
});
...
gulp.task('build', ['ts', 'views'], function(){
